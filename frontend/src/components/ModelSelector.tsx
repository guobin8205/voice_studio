      ...
      api.getDownloadStatus(m.name).then((res: any) => {
        if (res.sizes) {
          ...
          Object.entries(res.sizes).forEach(([sz, st]: [string, any]) => {
            ...
            updates[key] = st;
            if (st.status === 'completed' && !firstDownloaded) firstDownloaded = sz;
          });
        ...
        return (
          <div key={m.name} className="border-2 border-gray-200 rounded-xl bg-gray-50/50 px-4 py-3 space-y-2">
            <div className="flex items-center gap-3">
              <button
                onClick={() => {
                  if (isActive) selectedEntries.forEach(e => toggleModel(m.name, e.size));
                    ...
                    {!isDownloaded && !isDownloading && (
                      <button
                        ...
                        className={`text-sm px-1 transition-opacity ${
                          activeDownload !== null ? 'text-gray-300 cursor-not-allowed opacity-50' : 'text-violet-500 hover:text-violet-700'
                        }`}
